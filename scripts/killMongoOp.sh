...
get_mongo_currentops() {
	...
	for IP in ${MONGOS} ;do
		echo "db.currentOp({'ns' : /^${DB}./})" | $MONGO ${IP}:${PORT} | grep -v -e 'MongoDB shell version:' -e 'connecting to:' -e '^bye$'
	done
}
...
find_long_op () {
	#这个函数用来放在crontab周期性运行,查询大于指定时间(10分钟)的操作,发邮件
	#PORT=57017
	PORT=$1
	#DB='ICCv1'
	DB=$2
	...
	for IP in ${MONGOS} ;do
		...
		#记录符合条件的 Op
		#
		echo "db.currentOp({'ns' : /^${DB}./ , 'secs_running' : { '\$gt' : ${SECS_RUNNING} }})" | $MONGO ${IP}:${PORT} \
				| grep -v -e 'MongoDB shell version:' -e 'connecting to:' -e '^bye$' \
				| grep -v -e '{ "inprog" : \[ \] }' 
			echo
	done
...
if [ -z "$1" ];then
	...
	get_mongo_currentops ${PORT} ${DB} |grep -e '"ns"' -e '"secs_running"'
else
	if [ "$1" = 'kill_mongo_op_2' ];then
		$1 27017 umav3 3600
		$1 57017 ICCv1 3600
		$1 57017 mapreduce 3600
	elif [ "$1" = 'find_long_op' ];then
		find_long_op_result="$($1 57017 mapreduce 360)"
		ns_count=$(echo "${find_long_op_result}"|grep -e '"ns"'|wc -l)
		if [ $ns_count -ge 1 ];then
				#发邮件报警
				to_list='youngyang@icatholic.net.cn,willwan@icatholic.net.cn'
				subject="operations which took more than 360s found within db:mapreduce"
				content="${find_long_op_result}"
